...
const NotificationsPanel = ({ studentId, onUnreadCountChange }: NotificationsPanelProps) => {
              ...
              <div
                ...
                className={`relative flex gap-3 p-4 rounded-lg border transition-colors ${
                  n.is_read
                    ...
                    : "bg-amber-50/60 border-amber-200 dark:bg-amber-950/30 dark:border-amber-700"
                }`}
              >
                <div className="shrink-0 mt-0.5">
                  <AlertTriangle
                    className={`w-5 h-5 ${n.is_read ? "text-muted-foreground" : "text-amber-500"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className={`font-semibold text-sm ${n.is_read ? "text-muted-foreground" : "text-foreground"}`}>
                      {n.title}
                    </p>
                ...
                {!n.is_read && (
                  <button
                    ...
                    className="absolute top-2 right-2 p-1 rounded-full hover:bg-amber-100 dark:hover:bg-amber-900 transition-colors"
                    title="Mark as read"
                  >
